...
### 我的（`pages/my/my.vue`）
- 接入 `member` 云对象与登录授权，支持微信头像/昵称同步、默认头像兜底以及会员等级/积分展示。顶部卡片会实时显示当前等级、晋级进度和“最近变化”摘要，点击“查看明细”可进入积分流水页面。
- 提供每日签到、积分商城、优惠券中心、我的优惠券等组件，优惠券列表带骨架 loading、空态跳转按钮，便于快速体验真实运营场景；积分区域整合成一个信息盒子，不再在多个区域重复展示积分。
