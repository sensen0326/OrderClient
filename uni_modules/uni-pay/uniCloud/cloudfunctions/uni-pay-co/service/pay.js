...
class service {
	...
	getConfig(data = {}) {
		...
		if (config_directory && config_directory !== "default") {
			...
			if (!directoryConfig) {
				throw { errCode: ERROR[52003] };
			}
			// 合并配置
			realityConfig = Object.assign({}, config, directoryConfig);
		} else {
	...
	async paymentNotify(data = {}) {
		...
		let notifyPathSplit = path.substring(notifyPath.length).split("/");
		...
		// 初始化uniPayInstance
		let uniPayInstance = await this.initUniPayInstance({ provider, provider_pay_type, config_directory });
	...
	async createOrder(data = {}) {
		...
		// 拼接实际异步回调地址
		let currentNotifyPath = `${notifyPath}${provider}-${provider_pay_type}/${config_directory}`;
		let finalNotifyUrl = `${currentNotifyUrl}${currentNotifyPath}`;
		
		// 获取uniPay交易类型
		let tradeType = libs.common.getTradeType({ provider, provider_pay_type });

		let uniPayConifg = await this.getUniPayConfig({ provider, provider_pay_type, config_directory });
		// 初始化uniPayInstance
		let uniPayInstance = await this.initUniPayInstance({ provider, provider_pay_type, config_directory, app_auth_token, sub_app_id, sub_mch_id });
		// 获取支付信息
	...
	async getOpenid(data = {}) {
		...
		if (provider === "wxpay") {
			try {
				...
				let wxpayVirtualPayConifg = await this.getUniPayConfig({ provider: "wxpay-virtual", provider_pay_type: "mp", config_directory });
	...
	async getUniPayConfig(data = {}) {
		...
		if (config && config[provider] && config[provider][provider_pay_type]) {
			...
			if (provider === "wxpay" && sub_mch_id) {
				uniPayConfig = Object.assign({}, uniPayConfig, {
					subAppId: sub_app_id,
					subMchId: sub_mch_id,
				});
			}
			if (provider === "alipay" && app_auth_token) {
				uniPayConfig = Object.assign({}, uniPayConfig, {
	...
	/**
	 * 初始化uniPayInstance
	 * let uniPayInstance = await service.pay.initUniPayInstance({ provider, provider_pay_type, config_directory });
	 */
