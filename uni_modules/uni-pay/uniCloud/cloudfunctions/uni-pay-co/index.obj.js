...
module.exports = {
	...
	async createOrder(data) {
		...
		// 由于app_auth_token明文暴露在前端不安全，因此建议自己通过app_auth_token标识字段配合查询指定数据库表来获取app_auth_token（这部分逻辑需要自己实现）
		const getAppAuthToken = async (app_auth_token) => {
			// 这里可以写 app_auth_token 转换逻辑，比如前端传过来的app_auth_token是别名，然后再从数据库中获取真实的 app_auth_token
			return app_auth_token;
		}
		if (app_auth_token) {
			app_auth_token = await getAppAuthToken(app_auth_token);
		}
	...
	async getProviderAppId(data) {
		...
		// 注意，千万不要直接把 conifg 内的所有内容返回给前端
		let conifg = service.pay.getConfig(data);
		try {
			return {
				errorCode: 0,
