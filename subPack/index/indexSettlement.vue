<template>
	<view class="wrap">
	  <view v-if="cartChannel === 'takeout'" class="card address-card" @click="openAddressPopup">
		...
		<view v-else class="card-placeholder"> 请选择送餐地址 </view>
	  ...
	  <view v-else class="card table-card">
		...
		<view class="card-desc">
		  ...
		  <text v-if="tableInfo && tableInfo.distance" class="distance"> · 距离 {{ tableInfo.distance }}</text>
		</view>
	  </view>
	  ...
	  <view v-for="(item, index) in orderList" :key="index" class="u-flex list-box">
		...
		<view class="item-menu-name">
		  ...
		  <view class="u-flex u-row-between">
			...
			<view class="u-type-info u-font-22"> x{{ item.value }} </view>
		  </view>
		</view>
	  </view>
	  ...
	  <u-popup v-model="PopupShow" mode="bottom" height="80%" border-radius="14" closeable :mask-close-able="false">
		<view v-if="!PopupPage">
		  <view class="popup-inner">
			<u-image
			  src="https://mp-a83aee34-7c6d-40e3-a241-85ab45b7ff6e.cdn.bspapp.com/cloudstorage/static/menu/index-dining.png"
			  width="220"
			  height="180"
			></u-image>
		...
		<view v-else>
		  <view class="popup-inner">
			<u-image
			  src="https://mp-a83aee34-7c6d-40e3-a241-85ab45b7ff6e.cdn.bspapp.com/cloudstorage/static/menu/index-leave.jpg"
			  width="220"
			  height="180"
			></u-image>
	  ...
	  <u-popup v-model="invoicePopup" mode="bottom" height="60%" border-radius="20">
		<view class="invoice-popup">
		  ...
		  <u-checkbox :value="invoiceInfo.needInvoice" active-color="#EE2F37" @change="handleInvoiceCheck"
			>需要电子发票</u-checkbox
		  >
...
export default {
	data() {
	  return {
		...
		tags: ['不要辣', '少放辣', '多放辣', '少盐', '不要香菜', '不要花生', '少油', '多餐具'],
	...
	onLoad(param) {
	  ...
	  this.selectedAddressId = profile.selectedAddressId || (this.addressList[0] && this.addressList[0].id) || ''
			if (profile.channel === this.cartChannel) {
				const lastPeople = Number(profile.lastPeople) > 0 ? Number(profile.lastPeople) : 0
				this.form.people = this.cartChannel === 'dine_in' ? 0 : lastPeople
	...
	onShow() {
	  ...
	  if (latest && latest.userId) {
		const changed =
		  !this.memberProfile || this.memberProfile.userId !== latest.userId || this.memberProfile.points !== latest.points
		if (changed) {
 	...
 	methods: {
	  ...
	  calcFeeDetail() {
		const goodsTotal = this.orderList.reduce(
		  (sum, item) => sum + Number(item.price || 0) * Number(item.value || 0),
		  0
		)
	  ...
	  async refreshDeliveryQuote() {
		...
		const goodsAmount = this.orderList.reduce(
		  (sum, item) => sum + Number(item.price || 0) * Number(item.value || 0),
		  0
		)
	  ...
	  SelectPeople(param) {
		...
		uni.setStorageSync('peopleCount', this.form.people)
